...
const currentStreakLabel = el("currentStreakLabel");
const currentWinStreak = el("currentWinStreak");
const currentLossStreak = el("currentLossStreak");
const longestWinStreak = el("longestWinStreak");
const longestLossStreak = el("longestLossStreak");
const averageWinStreak = el("averageWinStreak");
const averageLossStreak = el("averageLossStreak");
const streakMessage = el("streakMessage");
const streakHero = el("streakHero");
const streakGauge = el("streakGauge");
...
function calculateStreaks(trades) {
  ...
  ordered.forEach((trade) => {
    if (trade.result === activeType) {
      activeLength += 1;
      return;
    }
    if (activeType) runs[activeType].push(activeLength);
    activeType = trade.result;
    activeLength = 1;
  });
  if (activeType) runs[activeType].push(activeLength);
  ...
  return {
    ...
    currentWin: activeType === "win" ? activeLength : 0,
    currentLoss: activeType === "loss" ? activeLength : 0,
    longestWin: Math.max(0, ...runs.win),
    longestLoss: Math.max(0, ...runs.loss),
    averageWin: average(runs.win),
    averageLoss: average(runs.loss),
  };
}

function renderStreaks(trades) {
  const streak = calculateStreaks(trades);
  currentStreak.textContent = streak.current;
  currentWinStreak.textContent = streak.currentWin;
  currentLossStreak.textContent = streak.currentLoss;
  longestWinStreak.textContent = streak.longestWin;
  longestLossStreak.textContent = streak.longestLoss;
  averageWinStreak.textContent = `${streak.averageWin.toFixed(1)} average`;
  averageLossStreak.textContent = `${streak.averageLoss.toFixed(1)} average`;

  streakHero.dataset.streak = streak.currentType || "none";
  streakGauge.style.setProperty("--streak-progress", `${Math.min(streak.current * 14, 100)}%`);

  if (streak.currentType === "win") {
    currentStreakLabel.textContent = "Win streak";
    streakMessage.textContent = streak.current >= 3
      ? `${streak.current} consecutive wins. Momentum is strong—keep risk and execution consistent.`
      : "Positive momentum. Continue following the same repeatable process.";
  } else if (streak.currentType === "loss") {
    currentStreakLabel.textContent = "Loss streak";
    streakMessage.textContent = streak.current >= 3
      ? `Risk alert: ${streak.current} consecutive losses. Consider reducing size and reviewing execution.`
      : "A loss is data. Review the setup before taking the next position.";
  } else {
    currentStreakLabel.textContent = "No streak";
    streakMessage.textContent = "Add completed win or loss results to begin streak tracking.";
  }
}
...
function render() {
  ...
  renderStats(trades);
  // Streaks always use the complete ledger so search and result filters cannot
  // accidentally turn non-consecutive trades into a false streak.
  renderStreaks(allTrades);
